...
def process_sample(sample, max_seg_length):
    """Process a single sample into normalized features/targets."""
    # Load image and get dimensions
    img = cv2.imread(sample['image_path'])
    img_height, img_width = img.shape[:2]
    
    # Crop and resize image patch using bbox
    x1, y1, x2, y2 = sample['bbox']
    crop = img[int(y1):int(y2), int(x1):int(x2)]
    crop = cv2.resize(crop, IMAGE_SIZE) / 255.0  # Normalize to [0,1]
    ...
    landmarks[::3] /= img_width   # Normalize x
    landmarks[1::3] /= img_height  # Normalize y
...
def create_tfrecord(dataset, output_path):
    """Serialize dataset into TFRecord format."""
